...
struct StringFormatNode : NodeContext
{
	...
	StringFormatNode(nosFbNodePtr node) : NodeContext(node)
	{
	...
	void OnPinValueChanged(nos::Name pinName, uuid const& pinId, nosBuffer value) override
	{
		if (pinName != NOS_NAME("Format"))
			return;
		const char* newFormat = InterpretPinValue<const char>(value.Data);
		if (Format == newFormat)
			return;
		Format = newFormat ? newFormat : "";
		Parse();
		UpdatePins();
	}
	...
	std::string FormatValue(Segment const& seg, NodeExecuteParams& pins)
	{
		nos::Name pinName(seg.Name.c_str());
		auto it = pins.find(pinName);
		std::string fmt = seg.Spec.empty() ? "{}" : "{:" + seg.Spec + "}";
		void* data = (it != pins.end() && it->second.Data) ? it->second.Data->Data : nullptr;
		...
		case ArgType::DOUBLE:
		{
			double v = data ? *static_cast<double*>(data) : 0.0;
			return std::vformat(fmt, std::make_format_args(v));
		}
		default:
		{
