...
{
	Frame base = (gs && gs->UpAxis == ofbx::UpVector_AxisZ) ? nos::graphics::UNREAL_SYSTEM
															: nos::graphics::GLTF_SYSTEM;
...
static graphics::TransformQ ConvertToTransformQ(RawTransform const& t, glm::dmat3 const& M, double scale)
{
...
struct ReadFBXTransformNode : NodeContext
{
	...
	Frame OutputFrame = nos::graphics::UNREAL_SYSTEM;
	bool AutoDetect = true;
	Frame SourceOverride = nos::graphics::GLTF_SYSTEM;
	// Source system read from the loaded file's header (basis + meters_per_unit).
	Frame Detected = nos::graphics::GLTF_SYSTEM;
	bool Loaded = false;
	...
	void UpdateOutputs(std::string const& selection)
	{
		...
		const glm::dmat3 S_tgt = nos::graphics::BasisMatrix(OutputFrame);
		const glm::dmat3 M = S_tgt * glm::inverse(S_src);
		const double scale = nos::graphics::UnitFactor(source, OutputFrame);
	...
	void PublishDetectedFrame()
	{
		...
		std::snprintf(buf, sizeof(buf), "%s up, %s, %g m/unit",
					  graphics::EnumNameSignedAxis(Detected.up()),
					  graphics::EnumNameHandedness(Detected.handedness()),
...
{
	NOS_BIND_NODE_CLASS_SAFE(NOS_NAME("nos.geometry.ReadFBXTransform"), ReadFBXTransformNode, fn);
	return NOS_RESULT_SUCCESS;
}
