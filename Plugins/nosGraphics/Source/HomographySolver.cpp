...
struct HomographySolverNodeContext : NodeContext
{
	...
	nosResult ExecuteNode(nosNodeExecuteParams* execParams) override
	{
		...
		src[0] = *reinterpret_cast<const glm::vec2*>(params.GetPinData<fb::vec2>(NOS_NAME("Source 1")));
		src[1] = *reinterpret_cast<const glm::vec2*>(params.GetPinData<fb::vec2>(NOS_NAME("Source 2")));
		src[2] = *reinterpret_cast<const glm::vec2*>(params.GetPinData<fb::vec2>(NOS_NAME("Source 3")));
		src[3] = *reinterpret_cast<const glm::vec2*>(params.GetPinData<fb::vec2>(NOS_NAME("Source 4")));
		dst[0] = *reinterpret_cast<const glm::vec2*>(params.GetPinData<fb::vec2>(NOS_NAME("Target 1")));
		dst[1] = *reinterpret_cast<const glm::vec2*>(params.GetPinData<fb::vec2>(NOS_NAME("Target 2")));
		dst[2] = *reinterpret_cast<const glm::vec2*>(params.GetPinData<fb::vec2>(NOS_NAME("Target 3")));
		dst[3] = *reinterpret_cast<const glm::vec2*>(params.GetPinData<fb::vec2>(NOS_NAME("Target 4")));
		...
		auto* out = params.GetPinData<fb::mat3>(NOS_NAME("Homography"));
		*out = reinterpret_cast<fb::mat3&>(H);
